...
class ArbitrageBot(SlackBot):
    ...
    BOT_NAME = "cryptobot"
    BOT_CHANNEL = (
        "GBYUB4398"  # Channel for arbitragebot to ask cryptobot for latest BTC price
    )
    ...
    def onPredict(self, cmd: Command):
        ...
        if cmd.user_name.lower() != "kaha":
            self.postMessage(
                channel, "Sorry bruv, I only hax for the crypto king.", thread
            )
        ...
        self.botPriceHash, self.nextBotUpdateTime = self._pollCryptoBot(
            channel, thread, self.coinList
        )
        if args[0] is not None:
            try:
                ...
                print(
                    "failed to parse args, hax cash = ${:0.2f}".format(
                        ArbitrageBot.HAX_CASH
                    )
                )
        ...
        while True:
            if time.time() > self.haxUntil:
                self._kaha_msg(
                    channel,
                    thread,
                    "Haxed ${:0.2f} over {} trades.".format(totalGainz, opportunities),
                )
            ...
            if bestGainz > 1.01:
                opportunities += 1
                print(
                    "best coin {}, up by {:0.2f}%".format(
                        bestCoin, (bestGainz - 1) * 100
                    )
                )
                buyAmt = round(self.hax_cash / self.botPriceHash[bestCoin], 6)
                print("${} worth is {}".format(self.hax_cash, buyAmt))
                self._kaha_msg(
                    channel, thread, "crypto buy {} {}".format(bestCoin, buyAmt)
                )
            else:
            ...
            if bestGainz > 1.01:
                ...
                self._kaha_msg(
                    channel, thread, "crypto sell {} {}".format(bestCoin, buyAmt)
                )
        ...
        while True:
            events = list(
                filter(
                    ...
                    and e.get("channel") == ArbitrageBot.BOT_CHANNEL
                    and e.get("username") == ArbitrageBot.BOT_NAME,
                    self.rtm_read(),
                )
            )
            if len(events) > 0:
            ...
            if time.time() > timeout:
                print(
                    "hax timed out waiting for {} reply!".format(ArbitrageBot.BOT_NAME)
                )
                self.postMessage(
                    errChannel,
                    "hax timed out waiting for {} reply :sob:".format(
                ...
                return [
                    self.botPriceHash,
                    self.nextBotUpdateTime + ArbitrageBot.BOT_REFRESH_TIME,
                ]
            time.sleep(0.1)
...
def _get_time_str(call_time):
    return (
        datetime.fromtimestamp(call_time, tz=utc)
